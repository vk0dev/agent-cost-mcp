...
> **Cost Guard para Claude Code.** `@vk0/agent-cost-mcp` lee tus logs JSONL locales de sesiones y responde a la pregunta que viene después de `/cost`: qué tool, branch, retry loop o runaway pattern está quemando tokens de verdad, y qué deberías cambiar a continuación. En `2.3.3`, el gasto diario positivo aunque sea pequeño se preserva en pricing y trend aggregation, así que la superficie de tendencia/anomalías ya no aplana ese gasto real en un falso zero-cost day. Cero nube. Sin API key.
...
- **Retry loops y runaway behavior:** detectar repeated turns, abandoned tool calls, anomaly days o branch-level burn patterns antes de que se acumulen en silencio. En `2.3.3`, el gasto diario positivo pequeño sigue visible en pricing/trend aggregation, así que la vista de tendencia/anomalías del operador es menos propensa a convertir gasto real en un falso zero-cost day.
...
- Nota de release actual: `2.3.3` preserva el gasto diario positivo pequeño en pricing/trend aggregation, así que la superficie actual de tendencia/anomalías mantiene visible el gasto real en vez de aplanarlo en un zero-cost day.
...
- `ccusage`, `claude-usage` o Claude-Code-Usage-Monitor pueden encajar mejor si tu prioridad es una experiencia reporting-first o monitor-first;
- CodeBurn puede encajar mejor si el burn-rate monitoring importa más que el detalle de local cost debugging;
