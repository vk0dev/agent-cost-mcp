...
> **给 Claude Code 用的 Cost Guard。** `@vk0/agent-cost-mcp` 会读取你的本地 JSONL 会话日志，回答 `/cost` 之后真正的问题：到底是哪一个 tool、branch、retry loop 或 runaway pattern 在烧 token，接下来该改什么。在 `2.3.3` 中，聚合后的日趋势与异常 baseline 改为锚定 assistant-row timestamps，因此旧的多日 JSONL 文件里较新的日期不再轻易从本地 operator 视图中消失。零云端。无需 API key。
...
- **retry loops 与 runaway behavior：** 在 repeated turns、abandoned tool calls、anomaly days 或 branch-level burn patterns 悄悄累积前就把它们抓出来。在 `2.3.3` 中，日趋势与异常 baseline 会绑定到 assistant-row timestamps，因此旧多日 JSONL 文件中的较新活动更不容易从 operator 视图中消失。
...
## 快速可视化 (native `/cost`、ccusage、cctally) vs Cost Guard forensics (`@vk0/agent-cost-mcp`)

Claude Code、ccusage 和 cctally 已经提供了有用的成本可视化。`@vk0/agent-cost-mcp` 解决的是下一层问题：forensics、attribution 和 guardrails。

### 什么时候用 native `/cost`、ccusage 或 cctally

- 你只想快速看一下当前会话的合计或 statusline 数字
- 你需要日次/周期用量 dashboard 或 burn-rate 汇总
- 你想要 Pro/Max 订阅的 quota forecast、cost-per-percent trends 或 threshold alerts (cctally)
...
用 native `/cost`、ccusage 或 cctally 做快速可视化；当下一个问题是 *哪个 tool、subagent 或 branch 烧掉了预算、继续跑会发生什么、agent 接下来该做什么* 时，再交给 `@vk0/agent-cost-mcp` — 从本地 JSONL 以机器可读的方式给出答案。
...
- 最新 release note：在 `2.3.3` 中，aggregated daily trend 与 anomaly baselines 会继续绑定 assistant-row timestamps，因此旧 multi-day JSONL logs 里的较新日期仍然可见。
...
| [`tokmon`](https://github.com/yagil/tokmon) | 你想包装一个正在运行的 **OpenAI `gpt-*`** 程序，实时查看它的 token cost —— tokmon 完全不支持 Claude/Anthropic，因此根本无法监控 Claude Code 会话。 | 通过在程序运行时代理其 OpenAI API 调用来实现 real-time cost monitoring。 | 开箱即用地支持 Claude Code，在 run 结束后离线地从 Claude Code JSONL 工作，提供 per-tool/session forensics、pricing-aware math 与 MCP-callable guardrails，而不是 live API proxying。 |
...
- 如果实时 burn-rate monitoring 或包装一个正在运行的 API 程序比 local post-run cost debugging detail 更重要，`CodeBurn` 或 `tokmon` 可能更合适 —— 但请注意 `tokmon` 仅支持 OpenAI `gpt-*` 模型，完全不支持 Claude Code 或 Anthropic。
