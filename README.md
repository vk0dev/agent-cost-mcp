...
> **Cost Guard for Claude Code.** `@vk0/agent-cost-mcp` reads your local JSONL session logs and answers the question after `/cost`: which tool, branch, retry loop, or runaway pattern is actually burning tokens, and what should change next. In `2.3.3`, tiny positive daily spend stays preserved through pricing and trend aggregation, so the trend/anomaly surface no longer drops small but real spend into a misleading zero-cost bucket. Zero cloud. No API key.
...
- **Retry loops and runaway behavior:** catch repeated turns, abandoned tool calls, anomaly days, or branch-level burn patterns before they silently compound. In `2.3.3`, tiny positive daily spend stays visible in pricing/trend aggregation, so operator trend and anomaly views are less likely to flatten small real spend into a false zero-cost day.
...
- If spend is split across parent and child sessions and you need the nested `subtreeCost` story, open [docs/subagent-tree-guide.md](./docs/subagent-tree-guide.md)
